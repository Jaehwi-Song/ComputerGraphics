...
def display():
    global cameraIndex, cow2wld, render_pos, draw_point, draw_point_pos, animStartTime
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if cursorOnCowBoundingBox:
                if -1 <= draw_point and draw_point < 6:
                    draw_point += 1
                    
                # to avoid initial click
                if 0 < draw_point and draw_point < 7:
                    draw_point_pos.append(cow2wld.copy())
